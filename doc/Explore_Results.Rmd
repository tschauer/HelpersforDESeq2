...
library(rtracklayer)
load(file.path(data_dir, list.files(path = data_dir, pattern = "^dmel")))
...
mito_genes <- get(res_names[1])$symbol[get(res_names[1])$chr %in% c("MT")]
```
...
for(res_name in res_names[c(2,3,1,4)]){
        ...
        plottingMA(res = get(res_name),
                   ...
                   selection_shadow = grepl("mATP", res_name),
                   xlims = c(0, 6),
                   ylims = c(-10,10),
                   x_axis_by = 2,
                   padj_cutoff = 0.01,
...
```{r message=FALSE, warning=FALSE,results=T, fig.align='center', fig.height=7, fig.width=7, dpi=200, out.width='75%'}
library(HelpersforDESeq2)
library(topGO)
...
all_genes <- factor(as.integer(get(res_name)$padj < 0.01 & get(res_name)$log2FoldChange > 0))
names(all_genes) <- get(res_name)$symbol

head(all_genes,10)

gt <- createGOTable(all_genes = all_genes,
                    shown_terms = 20,
                    min_signficant = 5,
                    select_ontology = "BP",
